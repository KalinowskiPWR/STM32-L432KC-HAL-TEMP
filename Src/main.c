...
int main(void)
{
  ...
  while (1)
  {
    if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK) { // Oczekiwanie na zakonczenie konwersji
	    ...
	    Temperature = -(( ( (float)(135 - 25)*(PomiarADC - TS_CAL1) ) / (TS_CAL2 - TS_CAL1) ) + 30);// Obliczenie temperatury
	    si ze = sprintf(data, "Temperatura %f stopni\n\r", Temperature); // Stworzenie wiadomosci do wyslania
	    //HAL_ADC_Stop(&hadc1);// Rozpoczecie nowej konwersji
